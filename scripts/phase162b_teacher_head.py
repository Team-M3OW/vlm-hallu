"""
Phase 162b (fix 3, variant, PRE-REGISTERED): box-free head with a TEACHER-AGREEMENT target.

Phase 162 used max-softmax gain as the pseudo-target and was 1 of 2 (Qwen2 ok, Qwen3 not), matching
the known asymmetry of the confidence signal. Variant target, still label-free and box-free:
    pseudo-target(c) = P_crop_c( argmax answer of uniform@600 )  -  P_uniform@300( same answer )
i.e. how much cropping at c increases the model's support for what IT answers with twice the budget
(self-distillation from the bar). Everything else identical. Same pre-registration as 162.

Original 162 header follows.
...
OUT=f"{D}/data/phase162b_pseudo_{WHICH}.jsonl"; B0,W,K=300,0.25,6
...
def main():
    ...
    for gi,r in enumerate(rows):
        ...
        gh,gw=r["grid"]; pu=answer(fit(img,B0)[0],ex["text"]); probs_u[gi]=pu
        p6=answer(fit(img,2*B0)[0],ex["text"]); teacher=int(np.argmax(p6)); conf_u[gi]=(teacher,pu[teacher],p6)
        base=np.where(G==gi)[0][0]
        for c in cand[gi]:
            cx,cy=((c%gw)+.5)/gw,((c//gw)+.5)/gh; pc=answer(fit(window(img,cx,cy,W),B0)[0],ex["text"])
            teacher,pu_t,_=conf_u[gi]; pseudo[base+c]=pc[teacher]-pu_t
    ...
    for gi,r in enumerate(rows):
        ...
        cx,cy=((j%gw)+.5)/gw,((j//gw)+.5)/gh; pcrop=answer(fit(window(imgs[gi],cx,cy,W),B0)[0],ex["text"]); p600=conf_u[gi][2]
