"""
Phase 83: DOES THE PRUNING RESULT REPLICATE? (SS14L on a second architecture)

SS14L on Qwen3-VL-2B: at 10% keep, a late-layer read-out loses NOTHING vs no pruning (-0.5pp) while
FastV's layer-2 ranking loses 22.0pp and scores BELOW random selection. Under the standard adopted
2026-09-16 that is provisional until it holds on a second model.

SHARP PREDICTION, fixed before the run: SS14K measured Qwen2-VL's early layers at gt_pct 0.620 --
far worse than Qwen3-VL's 0.456 -- so the layer-2 penalty should be LARGER here. If it comes out
SMALLER, the mechanism ("the question-conditioned signal does not exist yet at layer 2") is wrong.
...
OUT = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase83_pruning_qwen2vl.jsonl"
