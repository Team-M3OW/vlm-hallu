"""
Phase 227: DWA window scale on the cropping-supporting benchmarks.

The TextVQA diagnostic showed the W=0.25 window is the binding constraint on text-dense pages
(crop@300 == crop@600 exactly, W=0.5 recovers 20 points, random crop 13% vs DWA 51%). Document
pages (DocVQA) have the same geometry -- small text in a large page -- and the bar there is
genuinely resolution-starved, which is the regime where the crop literature reports gains.

Arms, same items, same DWA ridge cell (transferred weights, no refit), all crops at the grid's
answer budget (CROP_B=300 tokens), bar at 600:
  uniform@600   the equal-compute bar
  crop25@300    the grid's protocol
  crop35@300
  crop50@300
  crop50@600    same window, the bar's token count (separates window from budget)
  rand50@300    floor

Usage: phase227_dwa_scale.py <model-key> <docvqa|textvqa> [n]
"""
...
MODEL_ID, WT = MODELS[MK]; OUT = f"{D}/data/phase227_{MK}_{BK}.jsonl"
...
with open(OUT, "a") as f:
    for qid, img, q, gold, stratum, kind in items:
        ...
        rec = {"qid": qid, "gold": gold, "stratum": stratum, "preds": {}, "tokens": {}, "cell": [cx, cy]}
        for nm, im, tgt in (("uniform@600", img, 600), ("crop25@300", crop(img, cx, cy, 0.25), 300),
                            ("crop35@300", crop(img, cx, cy, 0.35), 300), ("crop50@300", crop(img, cx, cy, 0.50), 300),
                            ("crop50@600", crop(img, cx, cy, 0.50), 600), ("rand50@300", crop(img, rx, ry, 0.50), 300)):
