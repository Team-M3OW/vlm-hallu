"""
Phase 228: crop@300 vs uniform@300 -- the ViCrop-style equal-ANSWER-budget comparison.
...
MODEL_ID, WT = MODELS[MK]; OUT = f"{D}/data/phase228_{MK}_{BK}.jsonl"
...
with open(OUT, "a") as f:
    for qid, img, q, gold, stratum, kind in items:
        ...
        rec = {"qid": qid, "gold": gold, "stratum": stratum, "kind": kind, "probs": {}, "preds": {}, "tokens": {}, "cell": [cx, cy]}
        for nm, im in (("uniform@300", img), ("crop25@300", crop(img, cx, cy, 0.25)),
                       ("crop50@300", crop(img, cx, cy, 0.50)), ("rand25@300", crop(img, rx, ry, 0.25))):
