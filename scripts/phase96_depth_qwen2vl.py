"""
Phase 96: TESTING THE LOCATOR'S PREDICTION on Qwen2-VL-7B.

Phase 95 measured question-divergence per layer with NO labels and found the switch-on at L13-L14 on
Qwen2-VL (0.0046 at L12 -> 0.0130 at L13 -> 0.0442 at L14 -> 0.1779 at L16). On Qwen3-VL the same
measurement put it at L14->L16, which is EXACTLY where phase 87 found the pruning threshold.

So the locator makes a PREDICTION about a quantity we have never measured:

    Qwen2-VL's pruning damage should step around L13-L14.

This run tests it by sweeping the read depth directly. This is the experiment that decides whether
question-divergence is a usable, label-free way to find the read depth in an arbitrary model, or
whether the Qwen3-VL agreement was a coincidence.

    step at L13-L14   -> the locator works; "find your threshold with a few forward passes and no
                         ground truth" becomes a shippable tool
    step elsewhere    -> the agreement on Qwen3-VL was luck; report the locator as refuted

Block rescaled to the same fraction of the stack (L15-26 of 28).
"""
_OLD = """
...
import transformers.models.qwen2_vl.modeling_qwen2_vl as QM
from transformers import AutoProcessor, AutoModelForImageTextToText

MODEL_ID = "Qwen/Qwen2-VL-7B-Instruct"
OUT = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase96_depth_qwen2vl.jsonl"
BLOCK = list(range(15, 27))
