"""
Phase 161 (architecture fix 2): a SINGLE window spanning the top-k peaks -- removing the router.
...
OUT=f"{D}/data/phase161_span_{WHICH}.jsonl"; B0,W,RATIO,MINSEP=300,0.25,0.5,3
    ...
    with open(OUT,"w") as fout:
        for ex in ds:
            ...
            arms={"uniform@300":fit(img,B0)[0],"uniform@600":fit(img,2*B0)[0],
                  "head@0.25":fit(crop_frac(img,*head_box),B0)[0],"span":fit(crop_frac(img,*span_box),B0)[0],
            ...
            rec={"question_id_full":qid,"category":ex["category"],"label":label,"k":len(cells),
                 "span_box":[round(v,4) for v in span_box],"span_area":round((span_box[2]-span_box[0])*(span_box[3]-span_box[1]),4),
