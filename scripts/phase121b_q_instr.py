...
OUT = f"{D}/phase121b_prompts_{WHICH}.jsonl"
B0 = 300
...
def variants(text):
    ...
    instr = lines[-1] if has_instr else "Answer with the option's letter from the given choices directly."
    # Phase 121b: the ONE variant 121 did not run -- question + instruction, NO options. If V5 ~= V0
    # the options contribute nothing and the answer-emission point is the whole effect; if V5 << V0
    # the options matter too. V0 re-run in the same pass as the within-run control.
    return {"V0_full": text.strip(), "V5_q_instr": q + "\n" + instr}
