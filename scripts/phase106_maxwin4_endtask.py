...
OUT = f"{D}/phase106_maxwin4_{WHICH}.jsonl"
...
def maxwin4_proposals():
    """OOF window selection, identical to phase 105. Returns {qid: (cx, cy, coverage)}."""
    props = json.load(open(PROP))
    rows = [json.loads(l) for l in open(ATTN)]
    ...
    for k in range(K):
        ...
        best, bs = None, -1
        for s in range(28 - NW + 1):
            acc = np.mean([float(items[i][1][int(np.argmax(np.where(items[i][2],
                           items[i][0][s:s+NW].max(0), -1e9)))] >= COV_HIT) for i in tr])
            if acc > bs: bs, best = acc, s
        for i in te:
            A, c, rm, fx, fy = items[i]
            j = int(np.argmax(np.where(rm, A[best:best+NW].max(0), -1e9)))
            out[rows[i]["question_id_full"]] = (float(fx[j]), float(fy[j]), float(c[j]), int(best))
    return out, props
...
def main():
    ...
    print(f"max_win4 proposals: {len(mw)} items, coverage "
    ...
    with open(OUT, "w") as fout:
        for ex in ds:
            ...
            arms = {"maxwin4@0.25": window(img, mw[qid][0], mw[qid][1], W),
