"""
Phase 176c -- WHERE DOES THE IMAGE REACH THE TEXT? (primary run; 176b was its 12-item control)
ESTABLISHED by 176 + 176b on Qwen3 (n=12): masking the ANSWER token's image attention at all 28 layers
moves the output by KL 0.025 and flips 3% of answers; masking ALL TEXT ROWS flips 58% (KL 1.931). The
answer position does not read the image -- evidence arrives via the text tokens. 176b's per-layer profile
put transport at L6-L14 (peak L11 = 0.708) and ~zero after L18, i.e. BEFORE the L17-L20 read-out window.
THIS RUN makes that claim properly: 40 items, BOTH models, and PREFIX/SUFFIX masks so that "L11 is a
bottleneck" is separated from "blocking early blocks everything downstream":
    prefix L0..l  large KL only while l is inside the transport window
    suffix Ll..27 KL ~ 0 once l is past it  -> the depth at which transport is COMPLETE
SANITY (unchanged): mask-all must give large KL and flips; per-layer KL > 0.
...
def main(tag):
    ...
    for ex in ds:
        ...
        for l in range(0,NL,4):
            state["layers"]=set(range(0,l+1)); q=logits(inp)                      # block L0..l  -> transport BEFORE l+1
            rec["prefix"][str(l)]=[kl(base,q),int(int(torch.argmax(bl))!=int(torch.argmax(q[letters])))]
            state["layers"]=set(range(l,NL)); q=logits(inp)                       # block Ll..end -> transport AT/AFTER l
    ...
    json.dump(out,open(f"data/phase176c_transport_{tag}.json","w"))
    K=np.array([r["kl"] for r in out]); print("mean per-layer KL:"," ".join(f"L{l}:{K[:,l].mean():.4f}" for l in range(NL)))
    print(f"kl_all mean {np.mean([r['kl_all'] for r in out]):.3f}, flip_all {np.mean([r['flip_all'] for r in out]):.2f}")
    for nm in ("prefix","suffix"):
        ks=sorted(out[0][nm],key=int)
        print(f"  {nm} masks:  "+"  ".join(f"L{k}: KL {np.mean([r[nm][k][0] for r in out]):.3f} flip {np.mean([r[nm][k][1] for r in out]):.2f}" for k in ks))
