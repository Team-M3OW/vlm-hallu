"""
Phase 84: DOES THE ANSWER-FORMATION MECHANISM REPLICATE? (SS14I on Qwen2-VL-7B)

SS14I is Part III's centrepiece and is single-model. On Qwen3-VL-2B: the cropped and uncropped arms
are indistinguishable through L20, then separate by +15.3pp at L21 and stay separated; the failing
arms jump at L2 (the answer prior) and never improve; and the step appears ONLY where the window
actually covers (+29.7pp covered vs -7.9pp missed, a 37.6pp swing).

WHAT MUST REPLICATE -- fixed before the run
    NOT the specific layer index. L21 of 28 is one model's answer-formation depth and there is no
    reason another model shares it.
    THE SHAPE:  near-zero separation across most of the stack, then a step that persists.
    THE CONTROL: the step appears only where the head's window COVERS. This is the part that makes
    the result causal rather than correlational, and it is the part that must hold.

REJECTION RULE: if the covered/missed strata do not separate, the mechanism is Qwen3-VL-specific and
Part III is rewritten without it.

Qwen2-VL-7B has the same module layout (model.language_model with .norm, plus lm_head), so the lens
ports unchanged -- including the rule that the FINAL layer comes from model.logits and never from
norm(hidden_states[-1]), which cost a retraction once (SS12D).
"""
_OLD = """
...
OUT = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase84_lens_qwen2vl.jsonl"
B0, W = 300, 0.15
