"""
Diagnostic: why does cropping collapse on TextVQA when ViCrop reports gains?

ViCrop's protocol: baseline = the model at its FIXED native resolution (LLaVA-1.5 336px, ~576
tokens), crop = the attention region re-encoded at the SAME token count, no localise pass charged.
Ours: bar = uniform@600, crop = localise@300 + crop@300 (the localise pass is charged), so the crop
arm gets half the bar's tokens for the answer pass.

This runs the controlled arms on the same items at the same cell, to separate:
  uniform@300      is our bar's text reading mostly resolution or coverage?
  uniform@600      the bar
  crop25@300       our current protocol arm
  crop25@600       the crop at the SAME token count as the bar (ViCrop-style answer budget)
  crop50@600       W=0.5, to test the window-clipping failure mode
  rand25@600       the floor: a random crop at the same budget
300 TextVQA validation items, qwen3_2b, DWA cell from the transferred ridge.
"""
...
D = "/home/kavinder/ARNABI_ARSH/vlm-hallu"; OUT = f"{D}/data/diag_textvqa_crop.jsonl"
MODEL_ID = "Qwen/Qwen3-VL-2B-Instruct"; W = 0.25
...
def answer(img, q, target):
    src = fit_budget(img, target)[0]
    inp = build(src, q).to(model.device)
    p, t = B.score_item(model, pr, tok, inp, "open")
    nt = ntok(inp); del inp; torch.cuda.empty_cache(); return t, nt


rng = np.random.default_rng(225)
items = B.load("textvqa", 300)
...
with open(OUT, "a") as f:
    for qid, img, q, gold, stratum, kind in items:
        ...
        rec = {"qid": qid, "gold": gold, "preds": {}, "tokens": {}, "cell": [cx, cy], "rand_cell": [rx, ry]}
        for nm, im, tgt in (("uniform@300", img, 300), ("uniform@600", img, 600),
                            ("crop25@300", crop(img, cx, cy, 0.25), 300),
                            ("crop25@600", crop(img, cx, cy, 0.25), 600),
                            ("crop50@600", crop(img, cx, cy, 0.50), 600),
                            ("rand25@600", crop(img, rx, ry, 0.25), 600)):
            t, nt = answer(im, q, tgt)
            rec["preds"][nm] = t; rec["tokens"][nm] = nt
