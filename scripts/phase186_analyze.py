...
f=sys.argv[1]; rows=[json.loads(l) for l in open(f)]; n=len(rows); cat=np.array([r["category"] for r in rows]); rng=np.random.default_rng(186)
...
for t,m in [("single",cat=="direct_attributes"),("relational",cat=="relative_position"),("ALL",np.ones(n,bool))]:
    out=[]
    for nm,a,b in [("P1 450p-300",acc["ridge450p"],acc["ridge300"]),("P2 450p-bar",acc["ridge450p"],acc["uniform@600"]),("diag 600p-300",acc["ridge600p"],acc["ridge300"])]:
