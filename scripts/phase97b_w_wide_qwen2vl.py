"""
Phase 97b: the two WIDE windows phase 97 omitted, so the PRE-REGISTERED 5-value grid can be run.

PREREG_DCR_SIZER.md names W in {0.15,0.25,0.35,0.50,0.70}. Phase 97 launched before that file
existed and covered only the first three. Restricting Qwen3-VL to the common 3-value grid
INVERTED the sizer result there (+8.9pp -> +6.8pp, and -1.0pp against the fixed constant), so
the grid is not a free choice and the pre-registered one must actually be run rather than
narrowed to whatever is already on disk. Same items, same seed for the random arm, merged by
question_id_full.

Original phase 97 header follows.

Phase 97: THE COMPARISON WAS UNFAIR TO OUR OWN METHOD.

The "0 of 2 against the equal-compute baseline" verdict compares:

    Qwen3-VL  with W=0.25, selected in-sample and then VALIDATED HELD-OUT (phase 90)  -> +7.7pp
    Qwen2-VL  with W=0.15, the old window, never swept at all (phase 80b)             -> +3.1pp

Qwen2-VL was never given the better window. On Qwen3-VL, moving W=0.15 -> 0.25 moved the margin
against the bar from +4.7pp to +7.7pp. If Qwen2-VL moves similarly the second model lands near +6,
and "0 of 2" is a statement about an untuned arm rather than about the method.

PRE-REGISTERED, written before the run:

    W = 0.25 is NOT selected here. It was chosen on Qwen3-VL and held-out validated there
    (71.7% in-sample -> 71.6% held-out, folds pick it 99/100). This is a TRANSFER test: the
    same constant, no refitting, applied to a second architecture.

    PRIMARY   head@0.25 - uniform@600   (the equal-compute bar; DCR costs 2 passes at 300)
    DECISION  CI clear of zero  -> the claim becomes 1 of 2 and the rejection is premature
              CI spans zero     -> the rejection stands and W was not the missing piece
    CONTEXT   W = 0.35 is run only to show whether 0.25 sits on a plateau or a peak. It is NOT
              eligible to be reported as the method's window -- selecting it here would be
              exactly the in-sample selection phase 90 was built to catch.

Budget gate: tokens are MEASURED from image_grid_thw for every arm; >10% drift voids a contrast.
"""
"""
Phase 80b: DOES THE METHOD REPLICATE ON A SECOND ARCHITECTURE?

STANDARD ADOPTED 2026-09-16: every claim must hold on more than one model or be REJECTED. SS14D's
end-task result (+12.0pp over vanilla) is currently single-backbone. This is the test.

Qwen2-VL-7B, identical pipeline, proposals frozen out-of-fold in phase80a (coverage 35.1% -> 44.0%,
+8.9pp, against Qwen3-VL-2B's 39.3% -> 52.9%). Nothing is refit here.

REJECTION RULE, fixed before the run: if head does not beat argmax with a CI clear of zero, DCR is
a Qwen3-VL-2B artifact and the method section is cut.

The internal control carries over: on items where head and argmax pick the SAME cell the two arms
are the same computation and must split 0.0.
"""
_OLD = """
...
PROP = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase80a_qwen2vl_proposals.json"
OUT = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase97b_w_wide_qwen2vl.jsonl"
B0, W = 300, 0.15
WS = [0.5, 0.7]
...
def main():
    ...
    with open(OUT, "a") as fout:
        for ex in ds:
            ...
            for w in WS:
                arms[f"argmax@{w}"] = fit(window(img, *pp["argmax"], w), B0)[0]
                arms[f"head@{w}"]   = fit(window(img, *pp["head"], w), B0)[0]
                arms[f"rand@{w}"]   = fit(window(img, rcx, rcy, w), B0)[0]
                arms[f"oracle@{w}"] = fit(window(img, gcx, gcy, w), B0)[0]
