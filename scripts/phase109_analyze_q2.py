...
z = np.load(f"{D}/phase109_probe_qwen2.npz", allow_pickle=True)
X, ids, tot, POS = z["X"].astype(np.float32), list(z["ids"]), z["tokens_on_target"], list(z["positions"])
outs = {json.loads(l)["question_id_full"]: json.loads(l) for l in open(f"{D}/phase97m_merged_qwen2vl.jsonl")}
