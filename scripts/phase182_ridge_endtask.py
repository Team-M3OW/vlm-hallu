"""
Phase 182: DOES THE RIDGE DEPTH FILTER CONVERT? (§21 claims ridge == GBT as a ranker; this tests the end task)
...
OUT=f"{D}/data/phase182_ridge_{WHICH}.jsonl"; B0,W=300,0.25; Image.MAX_IMAGE_PIXELS=None
...
def placements():
    ...
    for gi,q in enumerate(rows):
        ...
        rm=rm.ravel()
        jt=int(np.argmax(np.where(rm,Pt[G==gi],-1e9))); jr=int(np.argmax(np.where(rm,Pr[m],-1e9)))
        out[q["question_id_full"]]={"gt":q["gt_box_frac"],
            "head":[float((jt%gw+.5)/gw),float((jt//gw+.5)/gh)],"ridge":[float((jr%gw+.5)/gw),float((jr//gw+.5)/gh)]}
    return out
def main():
    from huggingface_hub import snapshot_download; from datasets import load_dataset
    from transformers import AutoProcessor, AutoModelForImageTextToText
    PL=placements(); print(f"placements for {len(PL)} items",flush=True)
    ...
    with open(OUT,"w") as fout:
        for ex in ds:
            ...
            arms={"uniform@600":[fit(img,2*B0)],"head":[fit(win(img,*p["head"],W),B0)],
                  "ridge":[fit(win(img,*p["ridge"],W),B0)],
