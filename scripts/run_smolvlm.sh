...
# Launch: setsid nohup bash scripts/run_smolvlm.sh </dev/null >logs/q_smol.log 2>&1 & disown
unset HF_TOKEN HUGGING_FACE_HUB_TOKEN
cd /home/kavinder/ARNABI_ARSH/vlm-hallu || exit 1
mkdir -p logs
LOCK=/tmp/vlmhallu_gpu.lock; SELF=/tmp/vlmhallu_smol.lock; MINFREE=18000; STABLE=3
exec 8>"$SELF"; flock -n 8 || { echo "another run_smolvlm.sh is active"; exit 0; }
...
run(){
  ...
  echo "=== $(date +%H:%M:%S) $* ==="
  "$@" >"logs/smol_${tag}.log" 2>&1; rc=$?
  grep -aE "chance|cov 0|NL=|dumped|coverage|GUARD|Done ->|Traceback|Error|OutOfMemory|cells" "logs/smol_${tag}.log"|tail -10
  echo "--- EXIT $rc : $* ---"
  exec 9>&-
}
run python3 scripts/phase220b_layout_nonsquare.py smolvlm 24
echo "SMOLVLM PROBE DONE $(date +%H:%M:%S) -- inspect before dumping"
