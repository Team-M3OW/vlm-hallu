"""
Phase 223: is DWA just a ridge over the 28 log-attention values? END TASK, both checkpoints.

Coverage says yes: log-attention alone scores 0.606/0.579 against the deployed 63-feature set's
0.605/0.581. But coverage has over-reported twice in this project (the L20 exit looked free and cost
3.7 at the end task; the label-free normalisation closed 66-70% of the coverage gap and 0% of the
accuracy gap). So this is decided on answers, not on coverage.

Arms, all at the same budget (localise@300 + crop@300 = 600 tokens = the bar):
  bar            uniform@600, no crop
  dwa63          the deployed feature set: log A (28) + rank (28) + geometry (7)
  dwa28_log      log A only
  dwa28_raw      raw A only  (coverage says this one is worse; included so the log's value is measured)
PRE-REGISTERED: adopt log-only iff |dwa28_log - dwa63| CI excludes a 3-point loss on BOTH checkpoints.
"""
...
OUT=f"{D}/data/phase223_logonly_{WHICH}.jsonl"; NL=28
def cov(cx,cy,gt):
...
for i,r in enumerate(rows):
    ...
    RAW.append(a.T); LOG.append(np.log(a+1e-12).T)
    RNK.append((np.argsort(np.argsort(-a,axis=1),axis=1)/max(nc-1,1)).T)
    ...
    GEO.append(np.c_[np.log(nb+1e-12),fx,fy,np.sqrt((fx-.5)**2+(fy-.5)**2),
              np.minimum(np.minimum(fx,1-fx),np.minimum(fy,1-fy)),
              (xx.ravel()==gw-1).astype(float),(yy.ravel()==gh-1).astype(float)])
    Y.append(np.array([cov(float(fx[j]),float(fy[j]),r["gt_box_frac"]) for j in range(nc)]))
    G.append(np.full(nc,i)); meta.append((r["question_id_full"],gh,gw))
RAW=np.vstack(RAW);LOG=np.vstack(LOG);RNK=np.vstack(RNK);GEO=np.vstack(GEO)
...
def cells(X):
    ...
    for qid,gh,gw in meta:
        nc=gh*gw; s=P[off:off+nc]; off+=nc
    ...
    return out
ARMS={"dwa63":cells(np.c_[LOG,RNK,GEO]),"dwa28_log":cells(LOG),"dwa28_raw":cells(RAW)}
print("cells computed for",list(ARMS),flush=True)
...
def fit(img,target,refine=6,tol=0.08):
    ...
    return best[0]
def ans(im,t,B=300):
    inp=build(fit(im,B),t).to(model.device)
...
with open(OUT,"a") as f:
    for qid,gh,gw in meta:
        ...
        rec={"qid":qid,"category":e["category"],"label":lab,"probs":{"bar":ans(img,e["text"],600)}}
        for k,C in ARMS.items(): rec["probs"][k]=ans(crop(img,*C[qid]),e["text"],300)
