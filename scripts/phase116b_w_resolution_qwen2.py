"""
Phase 116b: CROSSING THE TWO AXES. Qwen2-VL-7B on HR-Bench 4k.

The coverage account (narrow windows hurt dispersed evidence, and do not matter for single targets)
holds on two models on V*Bench and on ONE model on HR-Bench and POPE. This crosses it: the
W-by-category interaction, measured on a second model on a second benchmark.

PRE-REGISTERED: W0.12 - W0.25 should be strongly negative on `cross` and near zero on `single`,
as on Qwen3-VL (-11.9pp [-23.9,-1.5] and +2.4pp [-3.6,+8.4]).
...
OUT = f"{D}/data/phase116b_w_resolution_hrbench_qwen2.jsonl"
WS = [0.06, 0.09, 0.12, 0.15, 0.25]
B0, NSEL = 300, 150
BLOCK = list(range(15, 27))  # same stack fraction as Qwen3-VL L16-26
