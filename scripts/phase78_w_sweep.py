...
The head fixes WHERE. W is still fixed at 0.15, inherited from a V*Bench CV that used the ARGMAX
proposer, not the head. Window size is a real trade-off and it is exactly the SS13B cliff:
  tokens_on_target = (target_area / W^2) * B0
A wider window is more likely to CONTAIN the target (coverage 52.9% at W=0.15 -> 84.8% at W=0.50)
and less likely to RESOLVE it (magnification 6.7x -> 2.0x), pushing it back under the cliff.

This run measures ACCURACY, not coverage, at the head's placement across W, plus the same sweep at
ORACLE placement. The decisive quantity is the ORACLE-PER-ITEM-W ceiling: if choosing the best W
per item barely beats the best FIXED W, there is nothing for a learned sizer to learn and the
direction is closed before anything is built. If the gap is large, the target signal exists and
the next question is whether the attention blob's spatial extent predicts it.

NOTE: coverage alone is a VACUOUS objective here -- W=1.0 covers 100% by construction. Only
accuracy can price this.
"""
_OLD = """
Phase 71b: THE END-TASK MEASUREMENT.
...
OUT = "/home/kavinder/ARNABI_ARSH/vlm-hallu/data/phase78_w_sweep.jsonl"
B0 = 300
...
def main():
    ...
    with open(OUT, "a") as fout:
        for ex in ds:
            ...
            for Wv in WS:
                arms[f"head@{Wv}"] = fit(window(img, *pp["head"], Wv), B0)[0]
                arms[f"oracle@{Wv}"] = fit(window(img, gcx, gcy, Wv), B0)[0]
            rec = {"question_id_full": qid, "category": ex["category"], "label": label,
                   "head_cov": pp["head_cov"], "argmax_cov": pp["argmax_cov"],
                   "gt_area_frac": (gt[2]-gt[0])*(gt[3]-gt[1]), "head_cell": pp["head"],
