...
average a fixed block. We show this choice dominates every other design decision, and that the depth these methods
read is not the depth at which the image reaches the answer. Masking text-to-image attention layer by layer
localises that transport to the first two thirds of the stack: blocking every layer from layer~16 of~28 onward
leaves the answer almost unchanged (KL $0.014$/$0.029$) and from layer~20 onward flips essentially no answers. The
boundary falls at the same fraction of depth, $0.57$, on a $36$-layer checkpoint as on the two $28$-layer ones,
entirely before the band these methods read.
Two policies follow from that measurement rather than from tuning. \textbf{Depth-Weighted Attention} (DWA) replaces
a hand-picked depth with one closed-form ridge solve over the whole depth profile, fitted from about fifty boxed
examples. \textbf{Adaptive Visual Resolution} (AVR) discards $90\%$ of visual tokens at the boundary, where they are
causally inert, and spends the freed compute on encoding resolution.
At matched compute DWA beats every published placement rule in all nine pooled comparisons. Across four
architectures --- native dynamic resolution, a fixed-token encoder, dynamic tiling and anyres --- depth
re-weighting beats the published single-block read-out on four of five checkpoints, by $+10.5$ to $+16.2$ points.
On Gemma-3 the incumbent is indistinguishable from random ($1.1\times$ chance coverage, $+0.5$ points) while
re-weighting the \emph{same} attention is worth $+16.2$. AVR converts a checkpoint's resolution headroom at rate
$1.01$ ($r{=}0.966$). Against models at their \emph{native} dynamic resolution --- $5.5\times$ and $7.2\times$ the
budget --- DWA is statistically indistinguishable on both using $14$--$18\%$ of the visual-token compute, whereas
every published rule falls $9.9$--$14.2$ points short.
\end{abstract}
