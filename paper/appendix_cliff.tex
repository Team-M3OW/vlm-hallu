...
\textbf{The logit lens: localisation is intact, the answer is not decodable.} Decoding every layer
with the logit lens (the final layer is read from the model's own logits; protocol and the
double-normalisation caveat in Appendix~\ref{app:mech}), no layer's decoded answer exceeds $52.9\%$
under the uniform encoding, against $92.6\%$ under the oracle crop. On items where the window covers
the target and the model still answers wrong ($n{=}36$), the correct option is the argmax at some
layer on $72.2\%$ of items, against $68.1\%$ where the window missed entirely; the apparent signal
at L20 ($41.7\%$) collapses to $0\%$ from L22 and matches what uninformative layers produce by
chance. Under the oracle crop the answer instead appears abruptly at L22, taking $90\%$ of the gain
in a single transition; the step replicates one layer later on Qwen2-VL-7B (L22--23) and appears
only where the crop covers the target ($+23.8$ points covered, $0.0$ missed). The oracle crop fixes
$94.4\%$ of the localised-but-wrong items at the same budget, with a median of $0.14$ merged tokens
